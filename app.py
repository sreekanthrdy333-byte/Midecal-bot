      ...
      if uploaded_file:
                ...
                if st.button("Analyze Scan"):
                              with st.spinner("Classifying & Routing..."):
                                                ...
                                                res_img, detections = registry.detect(category, np.array(image))
                                                if not detections:
                                                ...
                                                if detections:
                                                                      ...
                                                                      for d in detections:
                                                                                                st.markdown(f'<div class="detection-card"><span class="status-badge status-danger">Detected: {d["class"]}</span><br>Confidence: {d["confidence"]*100:.2f}%</div>', unsafe_allow_html=True)
                                                                                  ...
                                                                                  with chat_container.chat_message("user"):
                                                                                            ...
                                                                                            with chat_container.chat_message("assistant"):
                                                                                                          with st.spinner("Consulting..."):
                                                                                                                            response = gemini.chat_response(prompt, history=[{"role": m["role"], "parts": [m["content"]]} for m in st.session_state.messages[:-1]])
